...
get_sample_pseudotime = function(x, sample_metadata = NULL, 
  ...
  if (method == 'tscan'){
    ...
    TSCAN_result = myTSCANorder(sample_cluster,
  ...
  if (method == 'cca'){
    if (reduce) {
      pc = prcomp(t(x), scale = T)$x[,1:min(5, nrow(x), ncol(x))]
    }
    else {
      pc = t(x)
    }
    ...
    proj = sweep(pc, 2, res$xcenter, '-') %*% res$xcoef[,1, drop = F]
    ...
    return(list(sample_info = sample_info, slope = slope))
  }
